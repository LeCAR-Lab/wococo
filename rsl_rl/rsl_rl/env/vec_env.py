...
class VecEnv(ABC):
    ...
    @abstractmethod
    def symmetric_fn(self, obs: torch.Tensor, critic_obs: torch.Tensor, actions: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """Symmetric function to apply to the observations.

        Args:
            obs (torch.Tensor): Input observations. Shape: (num_envs, num_obs)

        Returns:
            torch.Tensor: Symmetric observations. Shape: (num_envs, num_obs)
        """
        raise NotImplementedError